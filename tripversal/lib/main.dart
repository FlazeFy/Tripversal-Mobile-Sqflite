...
class _NavBarState extends State<NavBar> {
  ...
  Widget build(BuildContext context) {
    final List<Widget> _widgetOptions = <Widget>[
      ...
      ContactPage(passUsername: passUsername),
    ];
                        ...
                        return InkWell(
                          onTap: () {
                            Navigator.push(
                              context,
                              PageRouteBuilder(
                                pageBuilder: (c, a1, a2) => GaragePage(passGarage: _garageList[index].garage_name, passUsername: widget.passUsername),
...
class _MyResPage extends State<MyResPage> {
    ...
    history.forEach((history){
      if(history['type'] == 'Car Rental'){
        setState((){
          ...
          historyModels.name = history['carname'];
          historyModels.location = history['location'];
          historyModels.driver = history['driver'];
  ...
  getAllHistoryDataTG() async {
    _historyList = <resvModel>[];
    var history = await _resvServices.readHistoryTG();

    history.forEach((history){
      if(history['type'] == 'Tour Guide'){
        setState((){
          var historyModels = resvModel();
          historyModels.idHistory = history['id_history'];
          historyModels.idUser = history['id_user'];
          ...
          historyModels.location = history['language'];
          historyModels.barcode = history['barcode'];
          historyModels.dateStart= DateTime.tryParse(history['dateStart']);
          historyModels.dateEnd= DateTime.tryParse(history['dateEnd']);
          historyModels.dateComment= DateTime.tryParse(history['dateComment']);
                            ...
                            child: Row(
                              children: [  
                                ...
                                Container(
                                  ...
                                  child: ElevatedButton(
                                    style: ButtonStyle(
                                      backgroundColor: MaterialStateProperty.all(const Color(0xFFe33b51)),
                                      padding: MaterialStateProperty.all(const EdgeInsets.all(8)),
                                    ),
                                    child: Column(
                                      mainAxisSize: MainAxisSize.min,
                                ...
                                Container(
                                  ...
                                  child: ElevatedButton(
                                    style: ButtonStyle(
                                      backgroundColor: MaterialStateProperty.all(const Color(0xFF13B402)),
                                      padding: MaterialStateProperty.all(const EdgeInsets.all(8)),
                                    ),
                                    child: Column(
                                      mainAxisSize: MainAxisSize.min,
                                                  ...
                                                  Container(
                                                    ...
                                                    child: RichText(
                                                      text: TextSpan(
                                                        children: [
                                                          TextSpan(
                                                            text: "${_historyList[index].name} ~ ${_historyList[index].driver}",
                                                            style: const TextStyle(
                                                              color: Color(0xFF808080),
                                                              fontWeight: FontWeight.w500,
                                                              fontStyle: FontStyle.italic,
...
class ContactPage extends StatefulWidget {
  const ContactPage({Key key, this.passUsername}) : super(key: key);
  final String passUsername;
...
class _ContactPageState extends State<ContactPage> {
  ...
  getAllSocial() async {
    ...
    contact.forEach((contact){
      if(contact['username'] == widget.passUsername){
      setState((){
                ...
                itemBuilder: (context, index){
                  ...
                  return InkWell(
                    ...
                    onTap: () { 
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => ChatPage(passGarageGuide: _contactList[index].garage_guide, passUsername: widget.passUsername)),
                      );
                    },                   
                  );
